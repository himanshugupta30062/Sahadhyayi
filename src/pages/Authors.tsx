...
import { Search, AlertCircle, RefreshCw, Star, Users, BookOpen, Sparkles, ArrowRight, TrendingUp, Library, SlidersHorizontal, X, MapPin, BadgeCheck } from 'lucide-react';
...
const Authors = () => {
  ...
  const averageRating = authors.length > 0
          ...
          <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 pt-6 sm:pt-8 pb-8 sm:pb-12 relative">
            ...
            >
              ...
              <p className="text-sm sm:text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed px-2">
                Explore refined author profiles, discover standout books, and open complete profiles with a single tap.
              </p>

              {/* Stats row */}
              <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 md:gap-10 mt-5 sm:mt-8">
                ...
                <div className="text-center hidden sm:block">
                  <div className="text-xl sm:text-2xl md:text-3xl font-bold text-foreground">{averageRating}</div>
                  <div className="text-[10px] sm:text-xs md:text-sm text-muted-foreground mt-0.5">Avg. Rating</div>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="bg-card/80 backdrop-blur-md rounded-xl sm:rounded-2xl shadow-[var(--shadow-card)] border border-border/60 p-3 sm:p-4 md:p-5">
                ...
                <div className="mt-3 sm:mt-4 grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <div className="rounded-xl border border-border/50 bg-background/70 px-3 py-2 text-left">
                    <div className="text-[11px] font-medium text-foreground">Rich profiles</div>
                    <div className="text-[10px] sm:text-xs text-muted-foreground">Browse bios, genres, and book previews in one glance.</div>
                  </div>
                  <div className="rounded-xl border border-border/50 bg-background/70 px-3 py-2 text-left">
                    <div className="text-[11px] font-medium text-foreground">Faster discovery</div>
                    <div className="text-[10px] sm:text-xs text-muted-foreground">Use search, genre filters, and sort controls to narrow your shortlist.</div>
                  </div>
                  <div className="rounded-xl border border-border/50 bg-background/70 px-3 py-2 text-left">
                    <div className="text-[11px] font-medium text-foreground">Reliable navigation</div>
                    <div className="text-[10px] sm:text-xs text-muted-foreground">Every card action now routes cleanly to the full author profile page.</div>
                  </div>
                </div>
              </div>
            </motion.div>
...
const FeaturedAuthorCard = ({ author, books }: { author: Author; books: Book[] }) => {
  ...
  const profilePath = `/authors/${slugify(author.name)}`;
      ...
      <Card className="overflow-hidden border-border/60 hover:shadow-[var(--shadow-elevated)] transition-all duration-300 hover:-translate-y-1.5 h-full bg-card">
          ...
          <CardContent className="p-3 sm:p-5 pt-3 sm:pt-4">
            ...
            <div className="grid grid-cols-2 gap-2 mb-3 sm:mb-4">
              <div className="rounded-xl bg-muted/50 px-2.5 py-2">
                <div className="text-[9px] sm:text-[10px] uppercase tracking-wide text-muted-foreground">Followers</div>
                <div className="text-xs sm:text-sm font-semibold text-foreground">{author.followers_count.toLocaleString()}</div>
              </div>
              <div className="rounded-xl bg-muted/50 px-2.5 py-2">
                <div className="text-[9px] sm:text-[10px] uppercase tracking-wide text-muted-foreground">Events</div>
                <div className="text-xs sm:text-sm font-semibold text-foreground">{author.upcoming_events}</div>
              </div>
            </div>
          </CardContent>
        </Link>
        ...
        <div className="px-3 sm:px-5 pb-3 sm:pb-5">
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <Button asChild variant="outline" size="sm" className="flex-1 text-[10px] sm:text-xs h-7 sm:h-8 border-border/60 group-hover:border-[hsl(var(--brand-primary)/0.4)] group-hover:text-[hsl(var(--brand-primary))] transition-colors">
              <Link to={profilePath}>
    ...
    <div className="group h-full">
      <Card className="h-full border-border/50 hover:shadow-[var(--shadow-card)] transition-all duration-300 hover:-translate-y-1 bg-card overflow-hidden">
        <Link to={profilePath} className="block">
          ...
          <div className="relative h-20 sm:h-24 bg-gradient-to-br from-[hsl(var(--brand-primary)/0.16)] via-[hsl(var(--brand-secondary)/0.12)] to-transparent overflow-hidden">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,hsl(var(--brand-primary)/0.22),transparent_55%)]" />
            <div className="absolute right-3 top-3 inline-flex items-center gap-1 rounded-full bg-background/90 px-2 py-1 text-[10px] font-medium text-foreground shadow-sm">
              <BadgeCheck className="h-3 w-3 text-[hsl(var(--brand-primary))]" />
              {author.verified ? 'Verified' : 'Community pick'}
            </div>
          </div>

          {/* Author header */}
          <div className="flex items-start gap-2 sm:gap-3 px-3 sm:px-4 -mt-5 sm:-mt-6 pb-2 sm:pb-3 relative z-10">
          ...
          <div className="grid grid-cols-3 gap-2 px-3 sm:px-4 pb-3 sm:pb-4">
            <div className="rounded-xl bg-muted/40 px-2 py-2 text-center">
              <div className="text-[9px] sm:text-[10px] uppercase tracking-wide text-muted-foreground">Books</div>
              <div className="text-xs sm:text-sm font-semibold text-foreground">{author.books_count}</div>
            </div>
            <div className="rounded-xl bg-muted/40 px-2 py-2 text-center">
              <div className="text-[9px] sm:text-[10px] uppercase tracking-wide text-muted-foreground">Fans</div>
              <div className="text-xs sm:text-sm font-semibold text-foreground">{author.followers_count.toLocaleString()}</div>
            </div>
            <div className="rounded-xl bg-muted/40 px-2 py-2 text-center">
              <div className="text-[9px] sm:text-[10px] uppercase tracking-wide text-muted-foreground">Events</div>
        ...
        {/* Actions */}
        <div className="flex gap-1.5 sm:gap-2 p-3 sm:p-4 pt-2 border-t border-border/30 bg-muted/20">
            <div className="hidden sm:block">
              <FollowButton authorId={author.id} size="sm" showText={false} />
            </div>
            <Button
