...
import { Pencil, Mic, Upload, Tag, BookOpen, type LucideIcon } from "lucide-react";
...
const actionButtons: ActionButton[] = [
  {
    ...
    icon: Pencil,
    onClick: (navigate) => navigate("/stories/upload"),
  },
  {
    label: "Record Voice Story",
    icon: Mic,
    onClick: (navigate) => navigate("/audio-journal"),
  },
  {
  ...
  },
  {
    ...
    icon: Tag,
    onClick: (navigate) => navigate("/tags"),
  },
  {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
              {actionButtons.map((btn) => {
                ...
                return (
                  <Card
                    key={btn.label}
                    className="cursor-pointer group hover:shadow-xl transition"
                    tabIndex={0}
                    onClick={() => btn.onClick(navigate)}
                    onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && btn.onClick(navigate)}
                    role="button"
                    aria-label={btn.label}
                  >
                    <CardContent className="flex flex-col items-center py-6">
                      <Icon
                        className="w-8 h-8 mb-2 text-amber-600 group-hover:scale-110 transition-transform"
                        strokeWidth={2.2}
                      />
                      <span className="font-semibold text-lg text-gray-900 text-center">
                        {btn.label}
                      </span>
                    </CardContent>
                  </Card>
