...
-- Create author_post_comments table (different name to avoid conflict)
CREATE TABLE public.author_post_comments (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  post_id UUID NOT NULL REFERENCES public.author_posts(id) ON DELETE CASCADE,
  user_id UUID NOT NULL,
  content TEXT NOT NULL,
  parent_comment_id UUID REFERENCES public.author_post_comments(id) ON DELETE CASCADE,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Enable RLS
ALTER TABLE public.author_post_comments ENABLE ROW LEVEL SECURITY;

-- RLS policies for author_post_comments
CREATE POLICY "Anyone can view comments on published posts" 
ON public.author_post_comments 
FOR SELECT 
USING (EXISTS (
...
-- Create author_post_reactions table
CREATE TABLE public.author_post_reactions (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  post_id UUID NOT NULL REFERENCES public.author_posts(id) ON DELETE CASCADE,
  user_id UUID NOT NULL,
...
-- Enable RLS
ALTER TABLE public.author_post_reactions ENABLE ROW LEVEL SECURITY;

-- RLS policies for author_post_reactions
CREATE POLICY "Anyone can view reactions on published posts" 
ON public.author_post_reactions 
FOR SELECT 
USING (EXISTS (
  SELECT 1 FROM public.author_posts 
  WHERE id = post_id AND is_published = true
...
CREATE TRIGGER update_author_post_comments_updated_at
  BEFORE UPDATE ON public.author_post_comments
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();
